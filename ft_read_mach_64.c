...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/30 07:20:16 by rojones           #+#    #+#             */
/*   Updated: 2017/06/30 08:28:11 by rojones          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_get_sct_sym_64(void *data, t_ofile *of, t_nlist_64 *nls, uint32_t i)
{
	t_section_64	section;

	memcpy(&section, (data + ft_get_section_offset_64(data,
			of->is_big_end, nls)), sizeof(t_section_64));
...
void	ft_unmask_ntype_64(t_nlist_64 *nls, t_ofile *ofile, uint32_t i,
		void *data)
{
	int				tmp;
	...
	if (tmp == 0x0e && nls->n_sect - 1 >= 0)
		ft_get_sct_sym_64(data, ofile, nls, i);
	if (tmp == 0x02)
